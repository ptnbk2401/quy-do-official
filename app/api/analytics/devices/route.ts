...
import { fetchDeviceData, fetchCountryData } from "@/lib/analytics-data";
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch both device and country data concurrently
    const [devicesData, countriesData] = await Promise.all([
      fetchDeviceData(startDate, endDate),
      fetchCountryData(startDate, endDate, 10), // Top 10 countries
    ]);
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "INTERNAL_ERROR",
          message:
            "An unexpected error occurred while fetching device and location data",
        },
      },
      { status: 500 }
    );
