...
import { fetchTrafficSources } from "@/lib/analytics-data";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sourcesData = await fetchTrafficSources(startDate, endDate);
    ...
    console.error("Analytics sources API error:", error);
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "INTERNAL_ERROR",
          message:
            "An unexpected error occurred while fetching traffic sources data",
        },
      },
      { status: 500 }
    );
